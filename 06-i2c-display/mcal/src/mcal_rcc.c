...
uint32_t mcal_rcc_get_apb1_clock_hz(void)
{
    /*
     * The clock setup uses APB1 /2 only when SYSCLK exceeds the STM32F103
     * 36 MHz APB1 limit. The HSI fallback keeps APB1 at HCLK/1.
     */
    return (g_system_clock_hz > STM32_MAX_APB1_HZ)
               ? (g_system_clock_hz / 2U)
               : g_system_clock_hz;
}

uint32_t mcal_rcc_get_apb1_timer_clock_hz(void)
{
    /*
     * STM32F1 timers receive 2 x PCLK1 when the APB1 prescaler is not 1.
     */
    return (g_system_clock_hz > STM32_MAX_APB1_HZ)
               ? g_system_clock_hz
               : mcal_rcc_get_apb1_clock_hz();
}

uint32_t mcal_rcc_get_apb2_clock_hz(void)
{
    /*
     * This project keeps APB2 at HCLK/1 in both the 72 MHz PLL
     * configuration and the 8 MHz HSI fallback configuration.
     */
    return g_system_clock_hz;
}

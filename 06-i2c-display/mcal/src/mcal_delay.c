...
void mcal_delay_busy_ms(uint32_t core_clock_hz, uint32_t delay_ms)
{
    ...
    /*
     * Every loop contains at least one CPU NOP in addition to loop overhead,
     * so the delay is intentionally conservative rather than cycle-exact.
     * This is used only for hardware settling during initialization.
     */

# 05 Timer PWM - Porting Guide

> **Purpose:** identify which assumptions in `05-timer-pwm` are board-specific, STM32F103-specific, Cortex-M3-specific, or application policy before changing hardware.
...
|---|---|
| PWM pin/channel | GPIO AF mapping and selected TIMx channel register fields |
| APB domain | timer clock doubling rule and actual bus prescaler |
| frequency | whether exact PSC/ARR decomposition exists within 16-bit limits |
| duty polarity | PWM mode, channel polarity, external load polarity |
| preload | update-event/shadow behavior on the target timer |
| different timer width | ARR/PSC field range and channel register layout |
...
[← Architecture](architecture.md) · [↑ Example README](../README.md) · [06 I2C display →](../../06-i2c-display/README.md)

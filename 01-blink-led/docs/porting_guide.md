# 01 Blink LED - Porting Guide

> **Purpose:** identify which assumptions in `01-blink-led` are board-specific, STM32F103-specific, Cortex-M3-specific, or application policy before changing hardware.
...
|---|---|
| LED pin/polarity | `board_pins.h`, GPIO port clock, CRL/CRH field, active level |
| Core clock | RCC configuration and SysTick integer reload constraint |
| Tick frequency | `BOARD_TIMEBASE_HZ` and time-service millisecond assumption |
| Low-power idle | wake source/debug behavior before replacing `NOP` with `WFI` |
| Different Cortex-M | SysTick/core register definitions and startup/vector assumptions |
...
[← Architecture](architecture.md) · [↑ Example README](../README.md) · [02 GPIO input interrupt →](../../02-gpio-input-interrupt/README.md)
